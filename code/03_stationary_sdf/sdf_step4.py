...
for ax, ix in zip(axes, ['SPI12','SPEI12']):
    ...
    for T in T_LIST:
        ...
        ax.plot(Ds, y, marker='o', ms=3.5, lw=1.8, color=tcolors[T], label=f'T = {T} yıl')
    ax.set_title(f'{"A" if ix=="SPI12" else "B"} — {ix} tabanlı SDF (Kırklareli)', fontsize=11, loc='left')
    ax.set_xlabel('Kuraklık süresi D (ay)')
    ax.set_xticks(Ds)
    ax.grid(axis='y', color='#E5E7EB', lw=0.6); ax.set_axisbelow(True)
    for sp_ in ['top','right']: ax.spines[sp_].set_visible(False)
axes[0].set_ylabel('Kritik kuraklık şiddeti S (−ΣSPI)')
axes[1].legend(fontsize=8.5, frameon=False, loc='upper left')
plt.tight_layout()
plt.savefig(W + "kirklareli_sdf.png", dpi=200, bbox_inches='tight')
