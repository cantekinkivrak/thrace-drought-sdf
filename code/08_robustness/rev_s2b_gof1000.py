...
DLO, DHI = (int(sys.argv[2]), int(sys.argv[3])) if len(sys.argv) > 3 else (1, 12)
...
if __name__ == '__main__':
    ...
    for (nm, ix, D), g in cs[cs.k==12].groupby(['name','indeks','D']):
        ...
        if len(x) >= 15 and (IXF is None or ix == IXF) and DLO <= D <= DHI: jobs.append((nm, ix, D, x.tolist(), 1000))
    print('k=12 hücre:', len(jobs))
    with Pool(8) as pool:
        res = [r for r in pool.map(cell_boot, jobs) if r]
    bt = pd.DataFrame(res)
    bt.to_csv(W+f'rev_gof_bootstrap_k12_{IXF or "all"}_{DLO}_{DHI}.csv', index=False)
