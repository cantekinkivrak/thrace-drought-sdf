...
for ax, fam in zip(axes, ['SPI','SPEI']):
    ...
    ax.annotate(f"T_AND ≈ {T:.0f} yıl\n(S={r.S:.0f}, D={int(r.L_ay)} ay)",
    ...
    ax.set_xlabel('Olay süresi D (ay)')
    ax.grid(color='#EEF1F3', lw=0.5); ax.set_axisbelow(True)
    for s_ in ['top','right']: ax.spines[s_].set_visible(False)
    ax.legend(frameon=False, fontsize=8.5, loc='upper left')
axes[0].set_ylabel('Olay şiddeti S')
plt.tight_layout()
plt.savefig(W + "tekirdag_copula_TAND.png", dpi=200, bbox_inches='tight')
