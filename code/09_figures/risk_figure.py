...
cb.set_label('Bileşik risk endeksi (0–1)', fontsize=9); cb.ax.tick_params(labelsize=8)
ax.set_xlim(25.8, 29.4); ax.set_ylim(40.2, 42.3)
ax.set_title('A — Bileşik kuraklık risk haritası (daire içindeki sayı = sıra)', fontsize=10.5, loc='left')
ax.set_xlabel('Boylam (°D)'); ax.set_ylabel('Enlem (°K)')
ax.set_aspect(1/np.cos(np.radians(41.2)))
for s_ in ['top','right']: ax.spines[s_].set_visible(False)
ax.annotate('* Lüleburgaz: sıcaklık homojenlik şüphesi — C1 bileşeni ihtiyatla', xy=(0.01, -0.13),
...
ax.set_title('B — Normalize bileşenler (0 = en düşük, 1 = en yüksek risk)', fontsize=10.5, loc='left')
for s_ in ['top','right','left','bottom']: ax.spines[s_].set_visible(False)
ax.tick_params(length=0)

plt.tight_layout()
plt.savefig(W + "risk_haritasi_F10.png", dpi=200, bbox_inches='tight')
