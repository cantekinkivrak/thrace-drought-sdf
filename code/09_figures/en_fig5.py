...
for st, c in [('Tekirdağ', C2), ('İpsala', C1)]:
    ...
    ax.plot(years, s100, color=c, lw=2.2, label=f'{st} — nonstationary model')
    # durağan referans
...
ax.set_title('A — Evolution of the 100-year drought severity (SPEI-12, D = 12)', fontsize=10.5, loc='left')
ax.set_xlabel('Year'); ax.set_ylabel('S₁₀₀ (effective 100-year severity)')
from matplotlib.lines import Line2D
h, l = ax.get_legend_handles_labels()
h.append(Line2D([0],[0], color=MUTE, lw=1.2, ls='--')); l.append('stationary model reference')
...
for i, (_, r) in enumerate(sub.iterrows()):
    ...
    ax.scatter([r.S100_1980], [i], color=MUTE, s=42, zorder=2, label='1980 climate' if i==0 else None)
    ax.scatter([r.S100_2024], [i], color=C2, s=52, zorder=3,
               edgecolor='white', linewidth=0.8, marker='o' if sig_ else 'D',
               label='2024 climate' if i==0 else None)
labels = [n + (' *' if n=='Kırklareli' else '') for n in sub.name]
ax.set_yticks(ypos); ax.set_yticklabels(labels, fontsize=9)
ax.set_title('B — S₁₀₀: 1980 → 2024 climate (SPEI-12, D = 12; ◆ = LRT p ≥ 0.05)', fontsize=10.5, loc='left')
ax.set_xlabel('S₁₀₀ (100-year drought severity)')
ax.annotate('* Kırklareli: extremes cluster in mid-record (2000–02); MK trend insignificant, NS slope fragile', xy=(0.01, -0.16), xycoords='axes fraction', fontsize=7.5, color='#6B7280')
ax.legend(frameon=False, fontsize=9, loc='lower right')
ax.grid(axis='x', color='#E5E7EB', lw=0.6); ax.set_axisbelow(True)
for s_ in ['top','right']: ax.spines[s_].set_visible(False)

plt.tight_layout()
plt.savefig(W + "figs_EN/Fig5_ns_synthesis.png", dpi=300, bbox_inches='tight')
