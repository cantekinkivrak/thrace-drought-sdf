...
for ax in axes[1]: ax.set_xlabel('Süre D (ay)')
for ax in axes[:,0]: ax.set_ylabel('Şiddet S')
leg = [Line2D([0],[0], color=SPI_C, lw=1.8, label='SPI-12'),
       Line2D([0],[0], color=SPEI_C, lw=1.8, label='SPEI-12'),
       Line2D([0],[0], color='#6B7280', lw=1.8, label='T = 100 yıl'),
       Line2D([0],[0], color='#6B7280', lw=1.4, ls='--', label='T = 10 yıl')]
fig.legend(handles=leg, ncol=4, loc='upper center', bbox_to_anchor=(0.5, 1.02), frameon=False, fontsize=9)
fig.suptitle('Durağan SDF eğrileri — 12 aylık ölçek, 8 istasyon (1966–2024)', y=1.06, fontsize=12)
plt.tight_layout()
plt.savefig(W + "sdf_8istasyon_k12.png", dpi=200, bbox_inches='tight')
...
for ax, fam in zip(axes, ['SPI','SPEI']):
    for k in [1,3,6,9,12]:
        ...
        ax.plot(med.index, med.values, color=kcol[k], lw=1.9, marker='o', ms=3.5, label=f'k = {k} ay')
    ax.set_title(f'{"A" if fam=="SPI" else "B"} — {fam} (bölge medyanı, T = 50 yıl)', fontsize=11, loc='left')
    ax.set_xlabel('Süre D (ay)'); ax.set_xticks([1,3,6,9,12])
    ax.grid(axis='y', color='#E5E7EB', lw=0.6); ax.set_axisbelow(True)
    for s_ in ['top','right']: ax.spines[s_].set_visible(False)
axes[0].set_ylabel('Şiddet S')
axes[1].legend(fontsize=9, frameon=False, loc='upper left', title='Birikim ölçeği')
plt.tight_layout()
plt.savefig(W + "sdf_olcek_duyarlilik.png", dpi=200, bbox_inches='tight')
